...
namespace Training
{
    class TrainingProgram
    {
        ...
        static void ShowVector(StreamWriter sw, double[] vector, int valsPerRow, int decimals, bool newLine)
        {
            for (int i = 0; i < vector.Length; ++i)
            {
                if (i % valsPerRow == 0) sw.WriteLine("");
                sw.Write(vector[i].ToString("F" + decimals).PadLeft(decimals + 4) + " ");
            }
            if (newLine == true) sw.WriteLine("");
        }

        static void ShowMatrix(StreamWriter sw, double[][] matrix, int numRows, int decimals, bool newLine)
        {
            for (int i = 0; i < numRows; ++i)
            {
                ...
                {
                    if (matrix[i][j] >= 0.0) sw.Write(" ");
                    else sw.Write("-");
                    sw.Write(Math.Abs(matrix[i][j]).ToString("F" + decimals) + " ");
                }
                sw.WriteLine("");
            }
            if (newLine == true) sw.WriteLine("");
